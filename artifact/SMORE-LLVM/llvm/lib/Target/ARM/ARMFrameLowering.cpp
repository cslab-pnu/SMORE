...
unsigned registerCnt = 0;
unsigned contextRegionSize = 0; //size of context region in stack frame
unsigned savedRegisterCnt = 0; //total number of saved register in stack
unsigned objectRegionSize = 0; //size of object region in stack frame
unsigned diffObjectRegionSize = 0; //difference between objectRegionSize and the value that can be expressed in thumb2 imm
unsigned bigFrameCnt = 0;

bool isTooBig = false; //checking whether the stack size is bigger than 4096
...
struct StackAdjustingInsts {
  ...
  unsigned emitDefCFAOffsets(MachineBasicBlock &MBB, const DebugLoc &dl,
...
//nicetauren
//calculate thumb2 immadiate values
std::pair<unsigned, unsigned> roundingThumb2Imm(unsigned imm){
...
//inserting redcanary prologue code
void insertOverlapPrologue(bool isARM, MachineFunction &MF,
                                 unsigned frameSize, const DebugLoc &dl,
                                 const ARMBaseInstrInfo &TII){
    unsigned src, base, imm, diff; //variables for copying add r7, #imm instruction

    bool isAlreadyPush = false; //checking whether push is double
    bool isAddR7SP = false; //checking whether instruction is add r7, sp, #imm
    
    unsigned numOfPushOperands = 0; //number of operands in one push instruction
    
    savedRegisterCnt = 0;
    for(MachineBasicBlock &MBB : MF){
        MachineBasicBlock::iterator MBBI = MBB.begin();
        MachineBasicBlock::iterator insertingCode;
        MachineBasicBlock::iterator addR7; // add r7, #imm instruction position
        
        //insert redcanary prologue code after PUSH instruction
        for(;MBBI != MBB.end(); ++MBBI){
            //insert redcanary prologue code after PUSH instruction
            if(MBBI->getOpcode() == ARM::t2STMDB_UPD && MBBI->getFlags() == MachineInstr::FrameSetup){
                bigFrameCnt = 0;

                numOfPushOperands = MBBI->getNumOperands() - 4;
                MBBI++;

                savedRegisterCnt += numOfPushOperands;
                objectRegionSize = 32+(frameSize-3)*32;
                contextRegionSize = 64 - (savedRegisterCnt * 4);
                
                //checking over stack size
                while(objectRegionSize > 4096){
                    bigFrameCnt++;
                    objectRegionSize -= 4096;
                }
                errs() << MF.getFunction().getName() << " - " << bigFrameCnt << "\n";

                std::pair<unsigned, unsigned> thumb2Imm = roundingThumb2Imm(objectRegionSize);
                objectRegionSize = thumb2Imm.first;
                diffObjectRegionSize = thumb2Imm.second;

                if(MF.getName() == "mem_manage_default"){
                    asmStr = "cpsid i\nsub sp, #32\nmovw r9, #0x0\nmovt r9, #0x2000\n"
                        "stm r9, {r4, r5, r6, r7, r8, r10, r11, lr}\nmov r9, #8\n";
                }else{
                    asmStr = "sub sp, "+std::to_string(contextRegionSize);
                }

                //asmStr += "\nmrs r9, psplim\nadd r9, #8\nmsr psplim, r9";

                if(objectRegionSize+diffObjectRegionSize > 32){
                  asmStr += "\ntt r9, sp\ncmp r9, #0\nbeq 1f\nbl placeRedzone\nb 2f\n1: movs r7, #";
                  asmStr += std::to_string(frameSize*32-96) + "\nbl shiftRedzone\n2:";
                } else {
                    asmStr += "\ntt r9, sp\ncmp r9, #0\nbeq 1f\nbl placeRedzone\n1:";
                }
                for(int i = 0; i<bigFrameCnt; i++){
                    asmStr += "\nsub sp, 4096";
                }
                if(objectRegionSize != 0){
                  asmStr += "\nsub sp, "+std::to_string(objectRegionSize);
                }
                if(diffObjectRegionSize != 0){
                    asmStr += "\nsub sp, "+std::to_string(diffObjectRegionSize);
                }
                asmStr += "\nbl calc_mem";
				asmStr += "\n";
                asmString = asmStr;
                ...
                BuildMI(MBB, MBBI, dl, TII.get(ARM::INLINEASM))
                    ...
                    .addImm(1)
                    .addExternalSymbol(constraints.data()); 
                
                //remove first inserting code
                if(isAlreadyPush){
                    insertingCode--;
                    insertingCode->eraseFromParent(); //erase first inserting code
                    insertingCode = MBBI;
                }else{
                  insertingCode = MBBI;
                  isAlreadyPush = true;
            ...
            if(MBBI->getOpcode() == ARM::t2STR_PRE && MBBI->getFlags() == MachineInstr::FrameSetup && MBBI->getOperand(0).getReg() == ARM::SP){
              bigFrameCnt = 0;
            ...
            if(MBBI->getOpcode() == ARM::t2ADDri && MBBI->getFlags() == MachineInstr::FrameSetup && MBBI->getOperand(0).getReg() == ARM::R7){
                addR7 = MBBI;
                src = MBBI->getOperand(0).getReg();
                base = MBBI->getOperand(1).getReg();
                imm = MBBI->getOperand(2).getImm();
                errs() << "Imm : " << imm << ", Imm*4 : " << imm*4 << "\n";
                
                isAddR7SP = true;
            }else if(MBBI->getOpcode() == ARM::tMOVr && MBBI->getFlags() == MachineInstr::FrameSetup && MBBI->getOperand(0).getReg() == ARM::R7){
                addR7 = MBBI;
                src = MBBI->getOperand(0).getReg();
                base = MBBI->getOperand(1).getReg();
                imm = 8;
        ...
        if(isAddR7SP){
            MachineBasicBlock::iterator eraseTarget = addR7;
            unsigned addCnt = 0;

            addR7 = insertingCode;
            imm = frameSize*32 - imm;

            if(savedRegisterCnt > 4){
                imm += 4;
            }
            ...
            std::pair<unsigned, unsigned> thumb2Imm = roundingThumb2Imm(imm);
            imm = thumb2Imm.first;
            diff = thumb2Imm.second;

            bool alreadyInst = false;
            while(addCnt > 0){
                if(!alreadyInst){
                    BuildMI(MBB, addR7, dl, TII.get(ARM::t2ADDri))
...
void ARMFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (MFI.hasVarSizedObjects())
    ...
    if(F.getName() == "reset_handler_default" || F.getName() == "hard_fault_handler" || F.getName() == "placeRedzone" || F.getName() == "restoreRedzone" || F.getName() == "shiftRedzone" || F.getName() == "unshiftRedzone" || F.getName() == "resetRedzone")
	  return;

    unsigned frameSize = MFI.getStackSize() / 32;
    insertOverlapPrologue(isARM, MF, frameSize, dl, TII);
}

//nicetauren
static void emitOverlapEpilogue(bool isARM, MachineFunction &MF, MachineBasicBlock &MBB,
                                 unsigned frameSize, const DebugLoc &dl,
                                 const ARMBaseInstrInfo &TII,
                                 unsigned MIFlags = MachineInstr::NoFlags,
                                 ARMCC::CondCodes Pred = ARMCC::AL,
  ...
  // for(MachineBasicBlock &MBB : MF){
      MachineBasicBlock::iterator MBBI = MBB.begin();
      MachineBasicBlock::iterator firstPop = MBB.begin();
      bool isOnePop = false; //checking whether pop is double
      
      for(;MBBI != MBB.end(); ++MBBI){
          if(MBBI->getOpcode() == ARM::t2LDMIA_RET){
              //remove first inserting code
              if(isOnePop){
                  isOnePop = false;
                  return;
              ...
              if(MF.getFunction().getName() == "mem_manage_default"){
              ...
              if(objectRegionSize != 0){
                asmStr += "add sp, "+std::to_string(objectRegionSize);
              }
              if (diffObjectRegionSize != 0){
                asmStr += "\nadd sp, "+std::to_string(diffObjectRegionSize);
              }
              asmStr += "\nadd r9, sp, #65\ntt r9, r9\nand r9, r9, #0xff\ncmp r9, #0x5\nbne 2f\nbl restoreRedzone\n2:";
              if(objectRegionSize+diffObjectRegionSize > 32){
                asmStr += "\nmov r7, "+std::to_string(frameSize*32-96)+"\nbl unshiftRedzone";
              }

              if(MF.getFunction().getName() != "mem_manage_default"){
                asmStr += "\nadd sp, "+std::to_string(contextRegionSize);
              }else{
                asmStr += "\nadd sp, 32\nmovw r9, 0x0\nmovt r9, 0x2000\n"
                    "ldm r9, {r4, r5, r6, r7, r8, r10, r11, lr}\ncpsie i\nbx lr";
              }
              asmString = asmStr;
          ...
          if(MBBI->getOpcode() == ARM::t2LDMIA_UPD && MBBI->getFlags() == MachineInstr::FrameDestroy){
              if(isOnePop){
                  isOnePop = false;
                  return;
              }
          ...
          if(MBBI->getOpcode() == ARM::t2LDR_POST && MBBI->getFlags() == MachineInstr::FrameDestroy){
			  asmStr = "";
...
void ARMFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (MF.hasWinCFI()) {
    ...
    unsigned frameSize = MFI.getStackSize() / 32;
    emitOverlapEpilogue(isARM, MF, MBB, frameSize, dl, TII);

    if(F.getName() == "placeRedzone" || F.getName() == "restoreRedzone" || F.getName() == "shiftRedzone" || F.getName() == "unshiftRedzone"){
        std::vector<MachineBasicBlock::iterator> eraseTarget;
        MachineBasicBlock::iterator mbbi = MBB.begin();
        for(;mbbi != MBB.end(); mbbi++){
            if(mbbi->getFlags() == MachineInstr::FrameSetup || mbbi->getFlags() == MachineInstr::FrameDestroy){
