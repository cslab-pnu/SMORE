...
void PEI::calculateFrameObjectOffsets(MachineFunction &MF) {
  ...
  int64_t StackSize = Offset - LocalAreaOffset;
    ...
    if(StackSize > 32){
      StackSize = 32 * ((StackSize-32)/32+4);
    }else{
      StackSize = 96;
    }
  ...
  NumBytesStackSpace += StackSize;
    ...
    std::set<int> calleeSavedIndices;
    for(const CalleeSavedInfo &I : CSI){
        calleeSavedIndices.insert(I.getFrameIdx());
    }
    unsigned cntCalleeSaved = calleeSavedIndices.size();
    unsigned movingByte = 64 - cntCalleeSaved*4;

    for(int i = 0, e = MFI.getObjectIndexEnd(); i != e; ++i){
        if(MFI.isFixedObjectIndex(i) || MFI.isDeadObjectIndex(i) || calleeSavedIndices.count(i))
        continue;

        int64_t currentOffset = MFI.getObjectOffset(i);

        currentOffset -= movingByte;

        MFI.setObjectOffset(i, currentOffset);
  }
}
